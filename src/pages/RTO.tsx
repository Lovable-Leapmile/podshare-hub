...
import { ArrowLeft, Package, Calendar, User, Phone, Key, CheckCircle, Clock } from "lucide-react";
...
export default function RTO() {
  ...
  useEffect(() => {
    // Filter reservations based on search query for active tab
  ...
  }, [pendingReservations, completedReservations, searchQuery, activeTab]);

  const loadRTOReservations = async (tab: TabType) => {
    ...
    try {
      ...
      toast.error(`Failed to load RTO ${tab} reservations`);
      if (tab === 'pending') {
